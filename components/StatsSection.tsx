...
export default function StatsSection() {
  return (
    <section className="w-full bg-[#F0F8F0] py-[15px]">
      <div className="max-w-[1318px] mx-auto flex flex-col lg:flex-row justify-between gap-[32px]">

        {/* LEFT SIDE */}
        <div className="flex flex-col gap-[24px] w-full lg:w-[558px]">

          <h3 className="text-[22px] leading-[32px] text-[#3A3A3A] font-normal">
            Fueling scalable growth with consistent results
          </h3>

          <div className="relative w-full lg:w-[558px] h-[609px] overflow-hidden shadow-[1px_1px_2px_rgba(0,0,0,0.2),inset_-4px_-4px_8px_rgba(255,255,255,0.1),inset_4px_4px_8px_rgba(0,0,0,0.45)]">
            <Image
        ...
        {/* RIGHT SIDE */}
        <div className="flex flex-col items-end gap-[26px] w-full lg:w-[450px]">

          {stats.map((item, index) => (
            <div
              key={index}
              className={`flex justify-between items-center w-full h-[58px] p-[10px]
              ${item.highlight ? "bg-gradient-to-r from-[#F4FFF7] via-[#E9FAEE] to-[#CFEEDD]" : ""}`}
            >

              <span className="text-[26px] leading-[38px] text-[#3A3A3A]">
                {item.value}
              </span>

              <span
                className={`text-[18px] leading-[27px] font-medium ${
                  item.highlight ? "text-[#3A3A3A]" : "text-[#8A8A8A]"
                }`}
              >
                {item.label}
              </span>
